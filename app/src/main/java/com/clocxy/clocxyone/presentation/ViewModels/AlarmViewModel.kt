package com.clocxy.clocxyone.presentation.ViewModels
...
import com.clocxy.clocxyone.domain.Functions.getCurrentHoursAndMinutes
...
class AlarmViewModel @Inject constructor(
    ...
    suspend fun saveClicked(
        ...
        if (isNew) {
            scheduleAlarm.execute(alarm) { result ->
                when (result) {
                    com.clocxy.clocxyone.domain.dataclass.Event.SUCCESS -> {
                    ...
                    com.clocxy.clocxyone.domain.dataclass.Event.FAILURE -> {
                        viewModelScope.launch {
                            withContext(Dispatchers.Main) {
                                Toast.makeText(
                                    context,
    ...
    fun deleteAlarm(alarm:Alarms){
        ...
        deleteAlarm.execute(alarm.id){event: com.clocxy.clocxyone.domain.dataclass.Event ->
            when(event) {
                com.clocxy.clocxyone.domain.dataclass.Event.SUCCESS -> {
                    viewModelScope.launch {
                        withContext(Dispatchers.Main){
